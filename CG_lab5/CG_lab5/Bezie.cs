            ...
            for (int i = 1; i < points.Count - 1; i += 2)
            {
                ...
                else
                {
                    ...
                    if (last == points.Count - 1)
                        ...
                        p3 = new PointF((points[last].X + p2.X) / 2, (points[last].Y + p2.Y) / 2);
                }

                List<PointF> result = calcaluteCubeСurve(p0, p1, p2, p3);
                for (int j = 1; j < result.Count; ++j)
                    g.DrawLine(pen, result[j - 1], result[j]);
                pictureBox_bezie.Refresh();
            }
        }

        private List<PointF> calcaluteCubeСurve(PointF p0, PointF p1, PointF p2, PointF p3)
        {
            float step = 0.01f;
            int step_draw = 101;
